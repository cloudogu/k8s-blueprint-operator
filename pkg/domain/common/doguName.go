...
func NewQualifiedDoguName(namespace DoguNamespace, simpleName SimpleDoguName) (QualifiedDoguName, error) {
	doguName := QualifiedDoguName{Namespace: namespace, Name: simpleName}
	err := doguName.Validate()
	if err != nil {
		return QualifiedDoguName{}, err
	}
	return QualifiedDoguName{Namespace: namespace, Name: simpleName}, nil
}

func (doguName QualifiedDoguName) Validate() error {
	var errorList []error
	if doguName.Namespace == "" {
		errorList = append(errorList, fmt.Errorf("namespace of dogu %q must not be empty", doguName.Name))
	}
	if doguName.Name == "" {
		errorList = append(errorList, fmt.Errorf("dogu name must not be empty: '%s/%s'", doguName.Name, doguName.Namespace))
	}
...
func (doguName QualifiedDoguName) String() string {
	return fmt.Sprintf("%s/%s", doguName.Namespace, doguName.Name)
}
